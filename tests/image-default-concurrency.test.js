...
async function run() {
  ...
  global.fetch = async (url, options = {}) => {
    ...
    const index = String(body.prompt || "").match(/Prompt (\d+)/)?.[1] || "0";
  ...
  assert.strictEqual(maxInFlight, 1, "default provider concurrency must be safe single-flight");
...
run()
  .then(() => console.log("image default concurrency ok"))
  .catch((error) => {
    console.error(error);
    process.exit(1);
  });
