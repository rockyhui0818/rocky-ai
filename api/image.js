...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return sendJson(res, statusCode, {
      ...
      provider: error.provider || publicProviderConfig(baseUrl, model)
    });
  }
};

async function runImageWorkflow({ payload, token }) {
  const apiKey = process.env.OPENAI_IMAGE_API_KEY || process.env.OPENAI_API_KEY;
  const baseUrl = normalizeImageBaseUrl(process.env.OPENAI_IMAGE_BASE_URL || process.env.OPENAI_BASE_URL || VERIFIED_IMAGE_BASE_URL);
  const model = normalizeImageModel(process.env.OPENAI_IMAGE_MODEL, baseUrl);

  if (!apiKey) {
    const error = new Error("Set OPENAI_IMAGE_API_KEY or OPENAI_API_KEY in your deployment environment.");
    error.code = "OPENAI_IMAGE_API_KEY_MISSING";
    error.statusCode = 500;
    error.provider = publicProviderConfig(baseUrl, model);
    throw error;
  }
  ...
  if (referenceBytes > MAX_REFERENCE_BYTES) {
    const error = new Error("上传参考图体积过大，请换用更小图片或等待前端压缩后再生成。");
...
module.exports.runImageWorkflow = runImageWorkflow;
